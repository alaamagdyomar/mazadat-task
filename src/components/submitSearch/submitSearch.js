...
const SubmitSearch = () => {
  ...
  const updateProductsRequest = () => {
    productsPriceFilter = productsState?.products?.filter(
      (minmaxPrice) =>
        minmax?.min <= minmaxPrice.price && minmaxPrice.price <= minmax?.max
    );
    ...
    //     productsColorFilter = [
    //       ...productsColorFilter,
    //       ...productsPriceFilter.filter(
    //         (product) => product.color === checkedColor.label
    //       ),
    //     ];
